...
try{
  ...
  $completed_order->update('orders','order_id',$id, array('order_status_code' => 3));
  Session::flash('completed', 'Order has been flagged as completed successfully!');
  exit(json_encode(['completed']));
}catch(Exception $e){
  die($e->getMessage());
}
