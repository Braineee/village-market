...
//check the session edit ID
if(!isset($_SESSION['edit_id'])){
  exit(json_encode(['error' => 'Product ID not specified.']));
  die();
}
...
try{
  ...
  //create the product
  $update_product = new Crud();
  $update_product
  ->update('products', 'product_id', $_SESSION['edit_id'], array(
  ...
  // empty the session edit
  $_SESSION['edit_id'] = '';

  Session::flash('product_updated', $product_name.' was updated successfully!');
  
  exit(json_encode(['success' => 'updated']));
  die();
